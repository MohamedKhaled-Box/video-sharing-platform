                ...
                <tbody>
                    @foreach ($users as $user)
                        <tr>
                                ...
                                <form class="ml-4 form-inline" method="POST" action="{{ route('channels.update', $user) }}"
                                    ...
                                    @if (auth()->user() != $user && !$user->isSuperAdmin())
                                        <button type="submit" class="btn btn-info btn-sm"><i class="fa fa-edit"></i>
                                            تعديل</button>
                                    @else
                                        <div class="btn btn-info btn-sm disabled"><i class="fa fa-edit"></i> تعديل </div>
                                    @endif
                                </form>
                                </form>
                            </td>
                            <td>
                                <form method="POST" action="{{ route('channels.delete', $user) }}"
                                    style="display:inline-block">
                                    @method('delete')
                                    @csrf
                                    @if (auth()->user() != $user && !$user->isSuperAdmin())
                            ...
                            <td>
                                <form method="POST" action="{{ route('channels.block', $user) }}"
                                    style="display:inline-block">
                                    @method('patch')
                                    @csrf
                                    @if (auth()->user() != $user && !$user->isSuperAdmin())
